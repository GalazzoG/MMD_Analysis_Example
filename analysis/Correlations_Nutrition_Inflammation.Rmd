...
```{r foodgroupscorr, include=F}

# Calculate Partial Spearman CC
food.groups.markrs.corr.obj<-partial_spearman_rho(Food_groups|biomarkers~ Metadata, adj.method = "TSBH")

# Creating dataframe with results
food.markrs.rho.ci<-food.groups.markrs.corr.obj$CI%>%
  mutate(color=ifelse(p.value<0.05, "darkred", "grey60"),
         alpha=ifelse(p.value<0.05, 0.8, 0.3),
         labels=ifelse(p.value<0.05, pairs, ""))

#save correlations dataframe
save(food.markrs.rho.ci, file=here("data/Robjects/Food_groups_Corr_obj.RData"))

```

```{r corrfoodplot, echo=FALSE, fig.width = 10,fig.fullwidth =T}

# Creating a volcano plot to visualise the results
ggplot(food.markrs.rho.ci, aes(rho, -log(p.value), color=color, alpha=alpha))+
  annotate(geom = "rect",xmin=-Inf, xmax=0, ymin=-log(0.05), ymax=Inf, fill="forestgreen", alpha=0.25)+# creating the red area in the plot
  annotate(geom = "label",x=-0.25, y=8, label="Anti-Inflammatory",fill="forestgreen", alpha=0.2)+# adding the area label
  annotate(geom = "rect",xmin=0, xmax=Inf, ymin=-log(0.05), ymax=Inf, fill="darkred", alpha=0.2)+# creating the green area in the plot
  ...
  theme_minimal()+
  ggtitle("Food groups VS. Inflammatory markers")
```

In terms of negative **anti-inflammatory** correlations we observe an association between:

- Alcoholic beverages with TNFa
- Red meat and offal with Z-score

The anti-inflammatory effect of alcohol consumption (in moderate amounts) is well known in the literature and reflected in the paper where the aDII was introduced. In this paper the inflammatory potential of alcohol is negative, meaning an anti-inflammatory effect. While the negative correlation of red meat and offal consumption, although above the threshold of significance, it should be interpreted with caution for the reasons explained in the note.

On the other hand we observe the **pro-inflammatory** effect of:

- Sweetened beverages and SAA
- Animal fat and IL6
- Oily fish with ICAM1
